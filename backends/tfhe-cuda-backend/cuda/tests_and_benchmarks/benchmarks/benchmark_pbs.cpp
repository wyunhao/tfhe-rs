...
(benchmark::State &st) {
  ...
  scratch_cuda_tbc_multi_bit_programmable_bootstrap<uint64_t, int64_t>(
      stream, gpu_index, (pbs_buffer<uint64_t, MULTI_BIT> **)&buffer, lwe_dimension,
  ...
  for (auto _ : st) {
    ...
    cuda_tbc_multi_bit_programmable_bootstrap_lwe_ciphertext_vector(
        stream, gpu_index, d_lwe_ct_out_array, d_lwe_output_indexes, d_lut_pbs_identity,
...
(benchmark::State &st) {
  ...
  scratch_cuda_programmable_bootstrap_tbc<uint64_t, int64_t>(
      stream, gpu_index, (pbs_buffer<uint64_t, CLASSICAL> **)&buffer, glwe_dimension,
