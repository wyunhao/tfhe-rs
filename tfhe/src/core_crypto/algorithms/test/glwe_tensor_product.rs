...
#[test]
fn test_glwe_encrypt_tensor_prod_decrypt() {
    ...
    let glwe_noise_distribution = DynamicDistribution::new_gaussian_from_std_dev(StandardDev(
        0.000000000000000315293223915005,
    ));
    ...
    while msg != u64::ZERO {
        ...
        for _ in 0..NB_TESTS {
            let mut plaintext_list = PlaintextList::new(
                0u64,
                PlaintextCount(input_glwe_secret_key.polynomial_size().0),
            );
            plaintext_list.as_mut()[0] = msg * delta;
            //PlaintextList::from_container([msg * delta] + [0u64
            // ;input_glwe_secret_key.polynomial_size().0]);
            ...
            let mut output_plaintext_list = PlaintextList::new(
                u64::ZERO,
                PlaintextCount(out_tensor_prod_ct.polynomial_size().0),
            );
            ...
            decoded
                ...
                .for_each(|(dst, src)| {
                    *dst = round_decode(*src.0, delta) % (msg_modulus * carry_modulus)
                });
            ...
            scaled_product
                ...
                .for_each(|(dest, &source)| {
                    *dest = u64::cast_from(source / <u64 as CastInto<u128>>::cast_into(delta)) >> 59
                    //log2(delta)
                });
